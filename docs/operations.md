...
| `AGENTSCRAPE_BROWSER_SESSION` | Pins every otherwise-ephemeral operation to one operator-managed `agent-browser` session. With the Agentbrowse provider, the stable session name maps to a durable Browser profile and can therefore reuse authentication established through the fleet's `browser` and `attention` workflow. Agentscrape never signs in or deletes the profile. An explicit `--session` still wins, and a name that is empty, over 128 characters, or outside `[A-Za-z0-9][A-Za-z0-9._-]*` is ignored in favor of the normal ephemeral session. |
| `AGENTSCRAPE_OWN_PINNED_SESSION` | Exact value `1` makes the process close the browser target behind `AGENTSCRAPE_BROWSER_SESSION` after each top-level operation while retaining its durable Agentbrowse profile and authentication. Agentscrape constrains every target it owns before navigation: reduced motion is requested and image, media, and font requests are blocked. An explicit `--session` remains caller-owned and is never closed or constrained by this setting. AgentStart uses this only for the serialized Agentbrain worker so an empty queue cannot retain a browser VM. |
